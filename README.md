...
- **@JsonIgnore**: Spring boot veriyi internet üzerinden göndermek için veriyi JSON fortmatına dönüştürür. Şimdi elinde bir User entitysi var ve içinde de Post listesi var, Post nesnesine bakıyoruz burada da User nesnesi var. Bu durumda ne olacak postu yazdır dediğimizde içindeki userı yazar, userın içinde de postu yazar, o postun içinde de tekrar userı yazar ne oldu infinite loop oldu. Bunun önüne geçmek için kullanıyoruz. Bu anotasyon ile bu alanı JSON çıktısına dahil etme yani serileştirme ve JSON girdisinden bu alanı okumaya çalışma yani deserilialization yapma diyoruz. Diyelim ki Post classında id, title, text bir de User nesnesi var. User nesnesinin üzerinde @JsonIgnore kullanırsak bir post ile get yaparsak bize id,title, text döner. 
